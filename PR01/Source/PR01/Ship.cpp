...
void AShip::FirePrimary(float val)
{
	...
	if (val && isReloaded) {
		...
		auto Projectile = GetWorld()->SpawnActor<AProjectile>(Projectile_BluePrint, 
			RootDirectionArrow->GetComponentLocation() + RootDirectionArrow->GetComponentRotation().Vector() * 500, 
			RootDirectionArrow->GetComponentRotation());
		Projectile->SetLifeSpan(2);
		Projectile->Fire(15000 + 5000 * current_engine_val / 100);
